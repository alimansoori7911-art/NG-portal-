        ...
        initPromise = (async () => {
            try {
                ...
                /* منبع نقش‌ها `/auth/me` است — بک‌اند صریحاً گفت
                   نمی‌خواهد فرانت به JWT تکیه کند، و از اسپک ۱۷
                   شمای `Profile` فیلد `roles` را **اجباری** دارد.

                   خواندن از JWT فقط یک تور نجات برای سرور قدیمی‌تر
                   است: `/auth/refresh` جز `access_token` چیزی نمی‌دهد،
                   پس اگر پروفایل نقش نداشته باشد ادمین با هر رفرش
                   صفحه از پنل بیرون می‌افتاد.

                   ترتیب اسپرد مهم است: `user` بعد می‌آید، پس نقشِ
                   پروفایل همیشه برنده است و توکن نادیده گرفته می‌شود.
                   وقتی مطمئن شدیم همه‌ی سرورها ≥۱۷ هستند، این و
                   `rolesFromToken` قابل حذف‌اند. */
                set({
